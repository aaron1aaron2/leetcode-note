...
- 202 - Happy Number [[link](https://leetcode.com/problems/happy-number)]
  ...
  - Tags: 我一開始使用 python list 的方式去儲存已出現的數字。使用 set 資料型態儲存可以有效改善判斷數字是否出現過的速度 (檢查從 O(k) 降到 O(1)（平均情況）)，這邊 record 的大小為 k 最多增長到 m(最壞情況)，在數理上平均來看在找尋 happy number 時的 O(k) = O(log n)。
